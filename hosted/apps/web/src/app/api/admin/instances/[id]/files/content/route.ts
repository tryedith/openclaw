...
// GET /api/admin/instances/[id]/files/content?path=memory/2026-02-20.md
export async function GET(
  ...
  try {
    ...
    console.error("[admin/files/content] Failed to resolve gateway target:", err);
  ...
  try {
    ...
    if (!result.ok) {
      console.error("[admin/files/content] Gateway error:", result.error);
      return NextResponse.json(
    ...
    console.error("[admin/files/content] Error:", err);
    return NextResponse.json(
